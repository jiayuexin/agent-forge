...
- **下一步 Top3**：真发解阻（改 scope/包名或协调占用）→ 可观测性 OpenTelemetry →（按需）其他安全/运维收尾。
...
- **R8 审计闭环**：Hub `GET/POST /api/audit` + Dashboard `/audit`；下发与配置变更自动写入；`runtime-client` 在 `local-command` 执行后自动上报 Hub；默认查询窗口 90 天。
