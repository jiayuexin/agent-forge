...
AgentForge 支持两种 Agent 形态，它们共享 `IAgent` 统一接口（详见 [01-核心设计.md §1.1](./01-核心设计.md#11-IAgent统一接口)），但生命周期、部署方式、状态管理和使用场景不同。
...
2. `PlannerAgent` 读取 `CapabilityRegistry`：
   ...
   - 远程有 `client-dev-machine`（ClientAgent，具备 `git-commit` 能力）
3. 生成计划：
