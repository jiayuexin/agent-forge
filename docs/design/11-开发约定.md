...
├── packages/
│   ├── types/          # 零运行时依赖的类型定义
│   ├── core/           # BaseAgent、Provider、ToolRunner、WASI Plugin 运行时、生成引擎
│   ├── sdk/            # AgentFramework、CapabilityExecutor、Pipeline、EventBus
...
`BaseAgent` 是抽象类，负责生命周期、中间件钩子、动态 Tool 源、Tool 执行事件与 Provider 调用。具体 Agent 只需实现 `doExecute`（以及可选的 `doStream`）。可下发 Plugin 不进入 Agent 进程，而是由 Core 的 Worker-backed WASI 运行器执行。
...
- Capability Hub 业务状态（Token、能力、审计、任务）使用 SQLite（`node:sqlite`）持久化；节点在线会话仍在内存中
- 日志使用 pino（生产环境 JSON，开发环境 pretty）
- 通过 W3C Trace Context 传播 OpenTelemetry 链路（设置 `OTEL_EXPORTER_OTLP_ENDPOINT` 后启用）
- `GET /api/v1/metrics` 暴露 Prometheus 指标
...
E2E 测试在 `main` / `dev` 的 PR 与 push 上运行。发布时为 `serve` 与 `dashboard` 模式构建 Docker 镜像。

### 5.1 npm 发布

- 根包 `@agentforge/root` 保持 private；`packages/*` 七包可发布，统一版本（当前 `0.1.0`）。
- 本地：`pnpm publish:dry-run`（需先 `pnpm build`）。
- CI：`.github/workflows/publish.yml`（`workflow_dispatch`，默认 dry-run；真实发布需 `NPM_TOKEN`）。
- 发布顺序与细节见 [DEPLOY.md](../ops/DEPLOY.md#npm-包发布)。
- **注意**：npmjs 上 `@agentforge/core` / `@agentforge/cli` 已被第三方占用，真实发布前需改名或取得所有权。
