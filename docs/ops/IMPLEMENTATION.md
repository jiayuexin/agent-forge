...
| 1.7 工具与能力类型 | `ToolDefinition`、`ToolHandler`、`PluginCapability`、`Middleware`       | 1.2  | `ToolDefinition` 使用 `JSONSchema` 拼写                              |
...
| 任务                   | 交付物                                                           | 依赖     | 验收标准                                                                                       |
| ---------------------- | ---------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------- |
| 2.1 BaseAgent          | `BaseAgent.ts` 抽象类                                            | 1.1      | 实现 `init`/`execute`/`stream`/`destroy`/`on`/`off`，状态机正确                                |
...
| 2.4 WASI Plugin 运行时 | `WasiPluginRunner.ts`、`PluginArtifactVerifier.ts`               | 1.7      | 签名 WASM 验签、Worker-backed WASI 执行、能力白名单                                            |
